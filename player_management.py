    ...
    while True:
        try:
            ...
            if 1 <= choice <= len(players):
                ...
                print(lang_manager.get("player_management.selected_player").format(player=selected_player))
                return selected_player
            elif choice == len(players) + 1:
                new_player = input(lang_manager.get("player_management.enter_player_name")).strip()
                if new_player:
                    players.append(new_player)
                    save_players(players)
                    print(lang_manager.get("player_management.player_added").format(player=new_player))
                    players = get_and_display_players_selection(lang_manager)
                else:
