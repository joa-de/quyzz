...
class GameController:
        ...
        try:
            ...
            for question_num in range(self.total_questions):
                ...
                user_answer = self.view.display_question(
                    question_num + 1, word_data, options, level
                )
                ...
                # Update mastery data
                self._update_mastery_data(
                    str(word_id), is_correct, mastery_data, player_name
                )

            # Calculate and log final score
            percentage = (score / self.total_questions) * 100
            logger.info(
                f"{player_name} completed quiz with score: {score}/{self.total_questions} ({percentage:.1f}%)"
            )
    ...
    def _update_mastery_data(
        self, word_id: str, is_correct: bool, mastery_data: Dict, player_name: str
    ):
